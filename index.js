...
function showData() {
    ...
    if (arr) {
        for (let i = 0; i < arr.length; i++) {
            ...
            addDiv.innerHTML = `<div class="top"> <img src="${arr[i].photo}" alt=""> 
            </div> <div> <div class="name"> <p class="p1"><b> ${arr[i].name} </b>
            </p> </div> <div class="details"> <P><b>Fathers Name :</b> ${arr[i].father_name} </P>
            <P><b>Email :</b> ${arr[i].email} </P>
            <p><b>Gender :</b> ${arr[i].gender} </p> 
            <p><b>Mobile No :</b> ${arr[i].mobile_no} </p> 
            <p><b>Skills : </b> ${arr[i].html}  ${arr[i].css} ${arr[i].java_script}  ${arr[i].node_js}</p> </div> </div>`;
